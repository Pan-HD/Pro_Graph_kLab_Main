...
int main(void) {
	...
	if (numSets == 1) {
		sprintf_s(inputPathName_ori, "./imgs_0816_2025_v1/input/oriImg_0%d.png", idSet);
		sprintf_s(inputPathName_tar, "./imgs_0816_2025_v1/input/tarImg_0%d.png", idSet);
		for (int j = 0; j < 2; j++) {
			if (j == 0) {
		...
		for (int i = 0; i < numSets; i++) {
			sprintf_s(inputPathName_ori, "./imgs_0816_2025_v1/input/oriImg_0%d.png", i + 1);
			sprintf_s(inputPathName_tar, "./imgs_0816_2025_v1/input/tarImg_0%d.png", i + 1);
			for (int j = 0; j < 2; j++) {
				if (j == 0) {
...
void multiProcess(Mat imgArr[][2]) {
	...
	errno_t err = fopen_s(&fl_fValue, "./imgs_0816_2025_v1/output/f_value.txt", "w");
	...
	errno_t err2 = fopen_s(&fl_maxFval, "./imgs_0816_2025_v1/output/maxFvalInfo_final.txt", "w");
	...
	errno_t err3 = fopen_s(&fl_printTree, "./imgs_0816_2025_v1/output/printed_tree.txt", "w");
		...
		for (int idxGen = 0; idxGen < GENERATIONS; idxGen++) {
			if ((idxGen + 1) % 100 == 0) {
				for (int idxSet = 0; idxSet < numSets; idxSet++) {
					...
					sprintf_s(imgName_pro[idxSet], "./imgs_0816_2025_v1/output/img_0%d/Gen-%d.png", idxSet + 1, idxGen + 1);
					imwrite(imgName_pro[idxSet], resImg_02);
					if (idxGen == GENERATIONS - 1) {
						vector<Mat> images = { resImg_02, imgArr[idxSet][1] };
						hconcat(images, res);
						sprintf_s(imgName_final[idxSet], "./imgs_0816_2025_v1/output/img_0%d/imgs_final.png", idxSet + 1);
