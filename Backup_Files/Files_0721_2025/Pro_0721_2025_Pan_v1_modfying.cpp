...
Mat diffProcess(const Mat& img1, const Mat& img2) {
	Mat out;
	add(img1, img2, out);
			...
			for (int idxGen = 0; idxGen < num_gen; idxGen++) {
				if ((idxGen + 1) % 10 == 0) {
					if (numSets == 1) {
						...
						sprintf_s(imgName_pro[0], "./imgs_0721_2025_v1/output/img_0%d/Gen-%d.png", idSet, idxGen + 1);
						imwrite(imgName_pro[0], resImg_01);
						if (idxGen == num_gen - 1) {
							vector<Mat> images = { resImg_01, imgArr[0][1] };
							hconcat(images, res);
							sprintf_s(imgName_final[0], "./imgs_0721_2025_v1/output/img_0%d/imgs_final.png", idSet);
