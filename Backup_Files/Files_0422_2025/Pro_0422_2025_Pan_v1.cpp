...
int main(void) {
	...
	if (numSets == 1) {
		sprintf_s(inputPathName_ori, "./imgs_0422_2025_v1/input/oriImg_0%d.png", idSet);
		sprintf_s(inputPathName_tar, "./imgs_0422_2025_v1/input/tarImg_0%d.png", idSet);
		for (int j = 0; j < 2; j++) {
			if (j == 0) {
		...
		for (int i = 0; i < numSets; i++) {
			sprintf_s(inputPathName_ori, "./imgs_0422_2025_v1/input/oriImg_0%d.png", i + 1);
			sprintf_s(inputPathName_tar, "./imgs_0422_2025_v1/input/tarImg_0%d.png", i + 1);
			for (int j = 0; j < 2; j++) {
				if (j == 0) {
...
void multiProcess(Mat imgArr[][3]) {
	...
	errno_t err = fopen_s(&fl_fValue, "./imgs_0422_2025_v1/output/f_value.txt", "a");
	...
	errno_t err1 = fopen_s(&fl_params, "./imgs_0422_2025_v1/output/params.txt", "a");
	...
	errno_t err2 = fopen_s(&fl_maxFval, "./imgs_0422_2025_v1/output/maxFvalInfo_final.txt", "a");
	...
	for (int idxGen = 0; idxGen < num_gen; idxGen++) {
		if ((idxGen + 1) % 10 == 0) {
			if (numSets == 1) {
				...
				sprintf_s(imgName_pro[0], "./imgs_0422_2025_v1/output/img_0%d/Gen-%d.png", idSet, idxGen + 1);
				imwrite(imgName_pro[0], resImg_01);
				if (idxGen == num_gen - 1) {
					vector<Mat> images = { resImg_01, imgArr[0][1] };
					hconcat(images, res);
					sprintf_s(imgName_final[0], "./imgs_0422_2025_v1/output/img_0%d/imgs_final.png", idSet);
				...
				for (int idxSet = 0; idxSet < numSets; idxSet++) {
					...
					sprintf_s(imgName_pro[idxSet], "./imgs_0422_2025_v1/output/img_0%d/Gen-%d.png", idxSet + 1, idxGen + 1);
					imwrite(imgName_pro[idxSet], resImg_02);
					if (idxGen == num_gen - 1) {
						vector<Mat> images = { resImg_02, imgArr[idxSet][1] };
						hconcat(images, res);
						sprintf_s(imgName_final[idxSet], "./imgs_0422_2025_v1/output/img_0%d/imgs_final.png", idxSet + 1);
