...
#define numDV 6 // the nums of decision-variables
...
int info_dv_arr[numDV] = { 8, 4, 4, 2, 3, 3 };
...
void import_para(int ko) {
    ...
    //erodeFlag = 0;
    dilateTimes = h[ko][3].fitness;
    aspectOffset = h[ko][4].fitness;
    contourPixNums = h[ko][5].fitness;
    // been modified
...
void multiProcess(Mat imgArr[][3]) {
    ...
    fprintf(fl_params, "Stat: %d%d, deci-vari: %d %d %d %d %d %d\n", gaussianBlurFlag, medianBlurFlag, threshVal, gaussianSize, circleOffset, dilateTimes, aspectOffset, contourPixNums);
